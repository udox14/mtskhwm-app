  ...
  const [liveData, logKeluar, kehadiranPegawai] = await Promise.all([
    db.prepare(`
      SELECT
        ...
        (SELECT COUNT(*) FROM izin_keluar_komplek WHERE tanggal = ? AND status = 'KEMBALI') as sudah_kembali,
        (SELECT COUNT(*) FROM izin_keluar_komplek WHERE tanggal = ?) as total_keluar
              ...
              <div key={i} className="flex items-center gap-3 px-4 py-2.5">
                ...
                <div className="text-right shrink-0">
                  ...
                  <p className={`text-[9px] font-medium ${r.status === 'BELUM KEMBALI' ? 'text-rose-500' : 'text-emerald-500'}`}>
                    {r.status === 'BELUM KEMBALI' ? 'Blm kembali' : r.jam_kembali?.substring(0, 5) ?? 'Kembali'}
                  </p>
                </div>
              </div>
            ))}
